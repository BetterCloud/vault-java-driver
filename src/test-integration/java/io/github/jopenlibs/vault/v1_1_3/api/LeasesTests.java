 ...
 *
 * <p>Unfortunately, it's not really possible to fully test these endpoints with the current
 * integration testing strategy.  The "generic" backend, used by the dev server, does not issue
 * leases at all.  You CAN obtain leases by using the PKI backend, but those aren't renewable:</p>
 *
 * <p>https://github.com/hashicorp/vault/issues/877</p>
 *
 * <p>Therefore, these revocation tests are basically just testing that the Vault server returns a
 * 204 status code.  Which isn't much of a test, since Vault routinely returns 204 even if you pass
 * a non-existent lease ID.</p>
 *
 * <p>In the future, we may be shifting to an integration testing approach that uses a "real" Vault
 * server instance, running in a Docker container (see:
 * https://github.com/BetterCloud/vault-java-driver/pull/25).  At that time, these tests should be
 * re-visited and better implemented.</p>
 *
